...
double gnps_cpp(NumericMatrix x, NumericMatrix y) {
  ...
  NumericVector x_mz = x(_, 0), y_mz = y(_, 0);
  NumericVector x_int = x(_, 1), y_int = y(_, 1);
  
  // Compute total intensity while avoiding duplicate m/z values
  ...
  // Filter out valid (non-NA) rows and store indices
  ...
  // Create kept m/z and intensity vectors
  ...
  for (int i = 0; i < l; i++) {
    int idx = keep_idx[i];
    double mz_x = x_mz[idx], mz_y = y_mz[idx];
    
    // Normalize intensity
    x_int_kept[i] = std::sqrt(x_int[idx] / x_sum);
    y_int_kept[i] = std::sqrt(y_int[idx] / y_sum);
    
    // Map unique m/z values to indices
    if (x_map.find(mz_x) == x_map.end()) x_map[mz_x] = x_count++;
    if (y_map.find(mz_y) == y_map.end()) y_map[mz_y] = y_count++;
    
    assignments(0, i) = x_map[mz_x];
    assignments(1, i) = y_map[mz_y];
  }
  ...
  // Solve Hungarian assignment problem (maximum matching)
