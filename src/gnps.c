...
static void *xmalloc(size_t n) {
  void *p = malloc(n);
  if (!p) GNPS_ABORT("malloc(%zu) failed", n);
  return p;
}

static void *xcalloc(size_t n, size_t s) {
  void *p = calloc(n, s);
  if (!p) GNPS_ABORT("calloc(%zu) failed", n * s);
  return p;
}
    ...
    GnpsCoreResult *out)
{
  ...
  int m = 0;
  for (int i = 0; i < n; i++) {
    if (!gnps_is_missing(x_mz[i])) { buf[m].key = x_mz[i]; buf[m].pos = i; m++; }
  }
  ...
  m = 0;
  for (int i = 0; i < n; i++) {
    if (!gnps_is_missing(y_mz[i])) { buf[m].key = y_mz[i]; buf[m].pos = i; m++; }
  }
  qsort(buf, (size_t)m, sizeof(KP), kp_cmp);
  double ys_sum = 0.0;
  for (int i = 0; i < m; i++) {
  ...
  double *keep_yin = (double *)xmalloc((size_t)l * sizeof(double));
  {
    ...
    for (int i = 0; i < n; i++) {
      if (!gnps_is_missing(x_mz[i]) && !gnps_is_missing(y_mz[i])) {
        keep_xmz[k] = x_mz[i]; keep_ymz[k] = y_mz[i];
        keep_xin[k] = x_int[i]; keep_yin[k] = y_int[i];
        k++;
      }
    }
  }
    ...
    GnpsJoinResult *out)
{
  ...
  int vx = 0, vy = 0;
  for (int i = 0; i < nx; i++) {
    if (!gnps_is_missing(x[i])) { xs_arr[vx].mass = x[i]; xs_arr[vx].idx = i; vx++; }
  }
  for (int i = 0; i < ny; i++) {
    if (!gnps_is_missing(y[i])) { ys_arr[vy].mass = y[i]; ys_arr[vy].idx = i; vy++; }
  }
  ...
  for (int i = 0; i < nx; i++) {
    if (gnps_is_missing(x[i])) { ox[pos] = i; oy[pos] = -1; pos++; }
  }
