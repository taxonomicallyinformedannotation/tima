...
#'
#' @importFrom tidytable across coalesce distinct filter fread group_by left_join na_if select where
#'
...
complement_metadata_structures <- function(
  ...
  met_2d <- fread(str_met,
    ...
    filter(!is.na(structure_inchikey_no_stereo) | !is.na(structure_smiles_no_stereo)) |>
    ## Avoid small discrepancies
    distinct(structure_inchikey_no_stereo,
      .keep_all = TRUE
    ) |>
    distinct(structure_smiles_no_stereo,
      .keep_all = TRUE
    )
  ...
  table_final <- df |>
    ...
    left_join(nam_i) |>
    left_join(nam_s) |>
    mutate(candidate_structure_name = coalesce(
