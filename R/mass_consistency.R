#' @title Graph-level adduct consistency
#'
#' @description
#' Helpers that score, filter and reconcile adduct hypotheses across the
#' feature pair-edge graph. Extracted from `annotate_masses.R` so the orchestrator
#' stays small and these algorithms can be unit-tested in isolation.
#'
#' Three layers:
#'   * [compute_feature_adduct_support()] : per (feature, adduct) neighbor count
#'   * [apply_adduct_consistency_filter()] : local "conditional/strict" pruning
#'   * [enforce_graph_adduct_consistency()] : connected-component reconciliation
#'
#' Plus state-key utilities that group adducts producing the same neutral
#' mass into a single conflict class.
#'
#' @include parse_adduct.R
#' @keywords internal
#' @name mass_consistency
NULL
