...
If no network was generated, simply use the `--network FALSE` argument for the `prepare_edges` and `prepare_features_components` steps. This will mimic a false network that allows to proceed to the next steps.

However, this will not help chemical weighting.

Use the `--mode manual` for `prepare_edges` and give a table containing your features as `input`, and corresponding column name as `1` argument.
