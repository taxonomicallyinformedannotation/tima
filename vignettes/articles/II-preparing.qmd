...
```{r prepare-features, eval=FALSE, include=TRUE}
...
```{r annotate-masses-1, eval=FALSE, include=TRUE}
...
- **Primary (strict graph):** each `feature_id` gets one primary ion species. Edges are kept only when both nodes are strongly supported, and consistent with the same neutral mass `M`.
- **Secondary (coverage):** additional non-conflicting ion-species hypotheses are kept as secondary.

For unresolved nodes in a supported component, TIMA also tests direct forms from the component mass `M` (for example `[M +/- loss + carrier]`) and keeps valid ones as weak support.
...
- `candidate_confidence_tier` (`supported_strong`, `supported_weak`, `baseline`)

### Spectral

```{r annotate-spectra-1, eval=FALSE, include=TRUE}
...
```{r prepare-gnps-1, eval=FALSE, include=TRUE}
...
```{bash run-sirius, eval=FALSE, include=TRUE}
...
```{r get-sirius, eval=FALSE, include=TRUE}
...
```{r prepare-sirius, eval=FALSE, include=TRUE}
...
```{r create-edges, eval=FALSE, include=TRUE}
tima::create_edges_spectra()
```

Prepare the edges:

```{r prepare-edges, eval=FALSE, include=TRUE}
tima::prepare_features_edges()
```

Create and prepare the components:

```{r create-components, eval=FALSE, include=TRUE}
tima::create_components()
```

```{r prepare-components, eval=FALSE, include=TRUE}
...
```{r prepare-taxa, eval=FALSE, include=TRUE}
tima::prepare_taxa()
```
...
```{r filter-annotations, eval=FALSE, include=TRUE}
