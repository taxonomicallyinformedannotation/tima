...
If no network was generated, simply use the `fake_features_edges` and `fake_features_components` steps instead of the `prepare_features_edges` and `prepare_features_components` steps.
...
Before running the corresponding code, do not forget to modify `config/user/prepare_features_edges.yaml`, `config/user/prepare_features_components.yaml`, and `config/user/prepare_features_classification.yaml` accordingly.
