...
Some pre-defined parameters are given in `params/default` directory.
...
targets::tar_visnetwork(names = starts_with("par_def"), targets_only = TRUE, degree_from = 8)
```
...
```{r echo=FALSE, message=FALSE, warning=FALSE, out.width="100%"}
targets::tar_visnetwork(names = starts_with("par_"), targets_only = TRUE, degree_from = 8)
```
...
The values you adapted in `params/prepare_params.yaml` were automatically propagated to all subfiles in `params/user`.
...
Before running the corresponding code, do not forget to modify `params/user/prepare_libraries_sop_merged.yaml`.
...
Before running the corresponding code, do not forget to modify `params/user/prepare_libraries_adducts.yaml`.
...
```{r}
source(file = "inst/scripts/prepare_libraries_is_lotus.R")
```
