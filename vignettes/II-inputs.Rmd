...
utils::download.file(url = paths$url$example_mgf_mini,
...
utils::download.file(
      url = paths$urls$example_spectral_lib,
      destfile = paths$data$source$libraries$isdb$neg
    )
paths$urls$isdb$pattern$pos <- "bla"
yaml::write_yaml(x = paths,file = file)
```

```{r eval=FALSE, include=TRUE}
source(file = "inst/scripts/get_isdb.R")
```
