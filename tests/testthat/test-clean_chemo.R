...
test_that("clean_chemo validates minimal_ms1_condition", {
  ...
  candidates_final <- 1; best_percentile <- 0.9; minimal_ms1_bio <- 0.5; minimal_ms1_chemo <- 0.5; minimal_ms1_condition <- "X"; compounds_names <- FALSE; high_confidence <- FALSE; remove_ties <- FALSE; summarize <- FALSE
  expect_error(clean_chemo(annot_table_wei_chemo=annot_table_wei_chemo, candidates_final=candidates_final, best_percentile=best_percentile, minimal_ms1_bio=minimal_ms1_bio, minimal_ms1_chemo=minimal_ms1_chemo, minimal_ms1_condition=minimal_ms1_condition, compounds_names=compounds_names, high_confidence=high_confidence, remove_ties=remove_ties, summarize=summarize), "OR or AND")
})
...
test_that("clean_chemo filters MS1 annotations with OR condition", {
  ...
  candidates_final <- 2; best_percentile <- 1; minimal_ms1_bio <- 0.8; minimal_ms1_chemo <- 0.8; minimal_ms1_condition <- "OR"; compounds_names <- FALSE; high_confidence <- FALSE; remove_ties <- FALSE; summarize <- FALSE
  result <- clean_chemo(annot_table_wei_chemo=annot_table_wei_chemo, candidates_final=candidates_final, best_percentile=best_percentile, minimal_ms1_bio=minimal_ms1_bio, minimal_ms1_chemo=minimal_ms1_chemo, minimal_ms1_condition=minimal_ms1_condition, compounds_names=compounds_names, high_confidence=high_confidence, remove_ties=remove_ties, summarize=summarize)
...
test_that("clean_chemo filters MS1 annotations with AND condition", {
  ...
  candidates_final <- 2; best_percentile <- 1; minimal_ms1_bio <- 0.8; minimal_ms1_chemo <- 0.8; minimal_ms1_condition <- "AND"; compounds_names <- FALSE; high_confidence <- FALSE; remove_ties <- FALSE; summarize <- FALSE
  result <- clean_chemo(annot_table_wei_chemo=annot_table_wei_chemo, candidates_final=candidates_final, best_percentile=best_percentile, minimal_ms1_bio=minimal_ms1_bio, minimal_ms1_chemo=minimal_ms1_chemo, minimal_ms1_condition=minimal_ms1_condition, compounds_names=compounds_names, high_confidence=high_confidence, remove_ties=remove_ties, summarize=summarize)
