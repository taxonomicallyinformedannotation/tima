...
test_that("parse_adduct parses simple positive adduct [M+H]+", {
  ...
  expect_true(unname(result["los_add_clu"]) > 0) # H adds mass
})
...
test_that("parse_adduct parses simple negative adduct [M-H]-", {
  ...
  expect_equal(unname(result["n_charges"]), 1)
  expect_equal(unname(result["charge"]), -1)
  expect_true(unname(result["los_add_clu"]) < 0) # H loss
})
...
test_that("parse_adduct parses sodium adduct [M+Na]+", {
  ...
  expect_equal(unname(result["n_mer"]), 1)
  expect_equal(unname(result["charge"]), 1)
  expect_true(unname(result["los_add_clu"]) > 20) # Na is ~23 Da
})
...
test_that("parse_adduct parses water loss [M+H-H2O]+", {
  ...
  expect_true(unname(result["los_add_clu"]) < 1)
})
...
test_that("parse_adduct parses doubly charged with modification", {
  ...
  expect_equal(unname(result["n_charges"]), 2)
  expect_true(unname(result["los_add_clu"]) < 2) # 2H - H2O
})
...
test_that("parse_adduct handles empty string without warning", {
  # Empty strings should be handled gracefully (common edge case)
  result <- parse_adduct("")
  expect_equal(unname(result["n_mer"]), 0)
})
...
#   result <- parse_adduct(adduct_string = "[M+H]+", regex = custom_regex)
#
