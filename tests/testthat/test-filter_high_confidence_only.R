...
test_that("filter_high_confidence_only validates threshold ranges", {
  df <- tidytable::tidytable(feature_id="F1", score_biological=0.9, candidate_score_pseudo_initial=0.9, score_weighted_chemo=0.9)
  expect_error(filter_high_confidence_only(df, score_bio_min=-0.1), "between 0 and 1")
  expect_error(filter_high_confidence_only(df, score_ini_min=1.1), "between 0 and 1")
  expect_error(filter_high_confidence_only(df, score_final_min=2), "between 0 and 1")
