...
test_that("is_spectrum_sanitized returns FALSE for duplicate mz within tolerance", {
  ...
  expect_false(is_spectrum_sanitized(sp, tolerance = 0.01, ppm = 0))
})
...
test_that("is_spectrum_sanitized returns FALSE for non-matrix", {
  expect_false(is_spectrum_sanitized(data.frame(mz = 1, intensity = 1)))
  expect_false(is_spectrum_sanitized(c(1, 2, 3)))
})
...
test_that("is_spectrum_sanitized never errors or returns NA on garbage input", {
  ...
  invisible(vapply(
    X = bad_inputs,
    FUN = function(x) {
      result <- is_spectrum_sanitized(x)
...
test_that("sanitize_spectrum_matrix sorts unsorted mz", {
  ...
  out <- sanitize_spectrum_matrix(sp)
...
test_that("sanitize_spectrum_matrix merges peaks within tolerance", {
  ...
  out <- sanitize_spectrum_matrix(sp, tolerance = 0.01, ppm = 0)
  expect_equal(nrow(out), 2L)
...
test_that("sanitize_spectrum_matrix returns input for empty or non-matrix", {
  ...
  expect_equal(sanitize_spectrum_matrix(empty), empty)

  not_matrix <- data.frame(mz = 1)
  expect_equal(sanitize_spectrum_matrix(not_matrix), not_matrix)
})
...
test_that("output of sanitize_spectrum_matrix always passes is_spectrum_sanitized", {
  ...
  out <- sanitize_spectrum_matrix(sp_messy, tolerance = 0.01, ppm = 0)
  if (nrow(out) >= 2L) {
    expect_true(is_spectrum_sanitized(out, tolerance = 0.01, ppm = 0))
  }
})
