...
make_spec <- function(mz, int) matrix(c(mz, int), ncol=2, dimnames=list(NULL,c("mz","int")))
...
test_that("calculate_similarity validates method", {
  expect_error(calculate_similarity(method="foo", query_spectrum=TEST_QUERY, target_spectrum=TEST_TARGET, query_precursor=1, target_precursor=2, dalton=0.01, ppm=10), "Invalid method")
})

test_that("calculate_similarity returns 0 for empty spectra", {
  empty <- matrix(numeric(), ncol=2)
  res <- calculate_similarity(method="cosine", query_spectrum=empty, target_spectrum=TEST_TARGET, query_precursor=1, target_precursor=2, dalton=0.01, ppm=10, return_matched_peaks=TRUE)
  expect_equal(res$score, 0.0)
  expect_equal(res$matches, 0L)
})

test_that("calculate_similarity rejects non-matrix input", {
  expect_error(calculate_similarity(method="cosine", query_spectrum=as.list(TEST_QUERY), target_spectrum=TEST_TARGET, query_precursor=1, target_precursor=2, dalton=0.01, ppm=10), "Spectra must be matrices")
})

test_that("calculate_similarity rejects negative tolerances", {
  expect_error(calculate_similarity(method="cosine", query_spectrum=TEST_QUERY, target_spectrum=TEST_TARGET, query_precursor=1, target_precursor=2, dalton=-0.01, ppm=10), "non-negative")
})
