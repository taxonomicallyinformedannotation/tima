...
create_test_annotation_data <- function(n_features = 5, n_candidates = 3) {
  ...
  df <- expand.grid(
    ...
    candidate_structure_inchikey_connectivity_layer = paste0("INK_", 1:n_candidates),
    stringsAsFactors = FALSE
  )
...
test_that("summarize_results removes ties when requested", {
  ...
  test_data$df$rank_final <- rep(c(1, 1, 2, 3, 4), length.out = nrow(test_data$df))
...
test_that("summarize_results renames score columns appropriately", {
  ...
  expected_score_cols <- c("score_initial", "score_biological",
                          "score_interim", "score_chemical", "score_final")
...
test_that("summarize_results trims whitespace", {
  ...
  if ("feature_id" %in% names(result)) {
    expect_false(any(grepl("^\\s|\\s$", result$feature_id, perl = TRUE), na.rm = TRUE))
  }
})
