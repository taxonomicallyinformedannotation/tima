...
test_that("chunked enrichment returns same values as unchunked", {
  df <- tidytable::tidytable(
    candidate_structure_inchikey_connectivity_layer = rep("IKC", 7),
    candidate_structure_smiles_no_stereo = rep("CC", 7),
    candidate_structure_exact_mass = as.character(seq_len(7)),
    candidate_structure_molecular_formula = paste0("OLD", seq_len(7))
  )

  st_file <- temp_test_path("chunk_st.tsv")
  met_file <- temp_test_path("chunk_met.tsv")
  nam_file <- temp_test_path("chunk_nam.tsv")
  cla_file <- temp_test_path("chunk_cla.tsv")
  npc_file <- temp_test_path("chunk_npc.tsv")
  ...
  withr::local_options(tima.complement_metadata_chunk_rows = Inf)
  out_full <- complement_metadata_structures(
    df,
    str_stereo = st_file,
    str_met = met_file,
    str_nam = nam_file,
  ...
  withr::local_options(tima.complement_metadata_chunk_rows = 2L)
  out_chunk <- complement_metadata_structures(
    df,
    str_stereo = st_file,
    str_met = met_file,
    str_nam = nam_file,
  ...
  expect_equal(
    out_chunk$candidate_structure_exact_mass,
    out_full$candidate_structure_exact_mass
  )
  expect_equal(
    out_chunk$candidate_structure_molecular_formula,
    out_full$candidate_structure_molecular_formula
  )
  expect_equal(
    out_chunk$candidate_structure_name,
    out_full$candidate_structure_name
  )
})

test_that("invalid chunk env var is ignored", {
  withr::local_envvar(TIMA_COMPLEMENT_METADATA_CHUNK_ROWS = "invalid")
  expect_true(is.infinite(tima:::.resolve_complement_chunk_rows(100L)))
})
